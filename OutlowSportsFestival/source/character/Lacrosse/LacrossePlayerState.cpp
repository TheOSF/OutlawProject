...
CharacterShotAttack* LacrosseState_PlayerControllShotAttack::CreateAttackClass(LacrossePlayer* t)
{
	...
	//ヒットイベント生成
	class LacrosseHitEvent :public DamageManager::HitEventBase
	{
	public:
		LacrosseHitEvent(LacrossePlayer* p) :m_pLacrossePlayer(p){}

		bool Hit(DamageBase* pDmg)
		{
			//自分の作っているダメージだった場合は何もしない
			if (pDmg->pParent->m_PlayerInfo.number == m_pLacrossePlayer->m_PlayerInfo.number)
			{
				return false;
			}
	...
	private:
		LacrossePlayer* m_pLacrossePlayer;
	};
