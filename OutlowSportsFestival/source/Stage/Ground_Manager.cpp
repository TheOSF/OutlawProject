...
const char* Ground_Manager::StageObjFileName = "DATA\\Stages\\Stage2\\StageObjData.txt";

Ground_Manager::Ground_Manager(UINT night_round) :
...
{
    //リソースにまとめて登録
    StageResourceLoadFaction::LoadStage2Object();
...
{
    //リソースから削除
    StageResourceLoadFaction::ReleaseStage2Object();
}


//-----------------Private----------------------//

void Ground_Manager::CreateStage()
{
    //土台なるステージをセット
    {
        iexMesh* pStageMesh;
        pStageMesh = new iexMesh("DATA\\Stages\\Stage2\\Ground.IMO");

        Matrix m;

        const float scale = 0.1f;

        MeshRenderer* R = new MeshRenderer(pStageMesh, true, MeshRenderer::RenderType::UseColor, MeshRenderer::GbufRenderType::NoTexture);
        ...
        {
            //キャラクタ専用の当たり判定ステージ
            MeshCollider* C2 = new MeshCollider(new iexMesh("DATA\\Stages\\Stage2\\Ground_ChrHit.IMO"), nullptr, true, CollisionManager::RayType::_Character);
            C2->SetWorldMatrix(m);
        ...
        {
            iexMesh* pHit = DefResource.Get(Resource::MeshType::Stage2_HitPhysic);
...
void Ground_Manager::RoundReset()
{
    //ラウンドカウント更新
    ++m_RoundCount;
...
void Ground_Manager::SetSunsetObject()
{
    //空を追加
    if (m_pSkyObject == nullptr)
    {
        m_pSkyObject = new SkyObject("DATA\\Stages\\Stage2\\Sky\\Sky.imo");
    }
...
void Ground_Manager::SetNoonObject()
{
    //空を削除
    if (m_pSkyObject != nullptr)
    {
...
void Ground_Manager::SetSunsetLight()
{
    //夕焼け時のライティングをセット
...
void Ground_Manager::SetNoonLight()
{
    //昼時のライティングをセット
    m_DirLight.param.color = Vector3(0.27f, 0.22f, 0.22f);
...
bool Ground_Manager::Msg(MsgType msg)
{
    switch (msg)
    {
    case _RoundReset:
